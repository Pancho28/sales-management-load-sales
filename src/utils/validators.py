...
def validate_sales_vs_payments(dfSales: pd.DataFrame, dfPayments: pd.DataFrame, local_name: str, env: str):
    """
    ...
    subject_prefix = "Sales Management Prueba - " if env == 'dev' else "Sales Management - "
    ...
    if sales_count == payments_count:
        ...
        send_email(
            subject=f"{subject_prefix}Éxito: {local_name}",
        ...
        send_email(
            subject=f"{subject_prefix}Descuadre de datos: {local_name}",
...
def validate_locals_list(locals_list: list, formatted_date: str, env: str) -> bool:
    """
    ...
    if len(locals_list) == 0:
        ...
        subject_prefix = "Sales Management Prueba - " if env == 'dev' else "Sales Management - "
        send_email(
            subject=f"{subject_prefix}Warning: Sin locales a procesar",
